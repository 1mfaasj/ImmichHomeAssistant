...
SERVICE_ENTITY_SCHEMA = vol.Schema({vol.Optional(ATTR_ENTITY_ID): vol.Any(cv.entity_id, [cv.entity_id])})
SERVICE_ENABLED_SCHEMA = SERVICE_ENTITY_SCHEMA.extend({vol.Required(ATTR_ENABLED): bool})
SERVICE_INTERVAL_SCHEMA = SERVICE_ENTITY_SCHEMA.extend({vol.Required(ATTR_SECONDS): vol.All(vol.Coerce(int), vol.Range(min=10, max=3600))})
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    ...
    def _target_entities(call: ServiceCall):
        ...
        ids = {requested} if isinstance(requested, str) else set(requested)
        return [entity for entity in _iter_entities() if entity.entity_id in ids]
    async def handle_next_image(call: ServiceCall):
        for entity in _target_entities(call): await entity.async_force_next_image()
    async def handle_set_shuffle_mode(call: ServiceCall):
        for entity in _target_entities(call): await entity.async_set_shuffle_mode(bool(call.data[ATTR_ENABLED]))
    async def handle_set_random_speed(call: ServiceCall):
        for entity in _target_entities(call): await entity.async_set_random_speed(bool(call.data[ATTR_ENABLED]))
    async def handle_set_refresh_interval(call: ServiceCall):
        for entity in _target_entities(call): await entity.async_set_refresh_interval(int(call.data[ATTR_SECONDS]))
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    hass.data.setdefault(DOMAIN, {})
    hass.data.setdefault(f"{DOMAIN}_entities", {})
    hub = ImmichHomeAssistantHub(hass=hass, host=entry.data[CONF_HOST], api_key=entry.data[CONF_API_KEY])
    if not await hub.authenticate(): raise InvalidAuth
