"""Image entities for the Immich integration."""
...
async def async_setup_entry(
    ...
    """Set up Immich image entities."""
    hub: ImmichHub = hass.data[DOMAIN][config_entry.entry_id]
    entities: list[ImageEntity] = [ImmichImageFavorite(hass, config_entry, hub)]
    ...
    if watched_album_ids:
        ...
        for album_id in watched_album_ids:
            ...
            if album_name:
                entities.append(ImmichImageAlbum(hass, config_entry, hub, album_id, album_name))
...
class BaseImmichImage(ImageEntity):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry, hub: ImmichHub, name: str, unique_id: str) -> None:
        self.hass = hass
        ...
        self._recent_asset_ids: deque[str] = deque(maxlen=max(self._no_repeat_window, 1))
    ...
    async def _async_refresh(self, force_asset_list: bool = False) -> None:
        try:
            ...
            if not self._asset_list:
                _LOGGER.warning("No assets are available for %s", self.name)
                return

            asset = self._select_next_asset()
            if not asset:
                _LOGGER.warning("No asset could be selected for %s", self.name)
                return
            ...
            if not image_bytes:
                _LOGGER.warning("Failed to download asset %s for %s", asset_id, self.name)
                return
...
class ImmichImageFavorite(BaseImmichImage):
    """Random image from the user's favorites."""

    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry, hub: ImmichHub) -> None:
        super().__init__(
            hass=hass,
            config_entry=config_entry,
            hub=hub,
            name="Immich favorite image",
...
class ImmichImageAlbum(BaseImmichImage):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry, hub: ImmichHub, album_id: str, album_name: str) -> None:
    ...
    def extra_state_attributes(self) -> dict:
        ...
        attrs.update({"album_id": self.album_id, "album_name": self.album_name})
        return attrs
